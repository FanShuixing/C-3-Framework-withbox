...
def loading_data():
    ...
    train_main_transform = own_transforms.Compose([
        own_transforms.RandomColorJitter(),
    ...
    train_set = SHHB(cfg_data.DATA_PATH, 'train', main_transform=train_main_transform, img_transform=img_transform,
                     gt_transform=gt_transform)
    train_loader = DataLoader(train_set, batch_size=cfg_data.TRAIN_BATCH_SIZE, num_workers=8, shuffle=True,
                              drop_last=True)

    val_set = SHHB(cfg_data.DATA_PATH, 'test', main_transform=val_main_transform, img_transform=img_transform,
                   gt_transform=gt_transform)
