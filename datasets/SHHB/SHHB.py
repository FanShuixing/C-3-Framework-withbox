...
class SHHB(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        wh, ind, reg_mask, reg = self.get_wh_gt(bboxes)
    ...
    def read_image_and_gt(self, fname):
        ...
        if img.mode == 'L':
            img = img.convert('RGB')
        img = img.resize((cfg_data.STD_SIZE[1], cfg_data.STD_SIZE[0]))
        den, hm_mask = get_density(os.path.join(self.root_dir, fname[1]), os.path.join(self.root_dir, fname[0]),
                                   w=cfg_data.STD_SIZE[1], h=cfg_data.STD_SIZE[0])
    ...
    def get_bboxes(self, fname):
        ...
        with open(os.path.join(self.root_dir, fname[0])) as fr:
            ...
            bboxes = np.zeros((num_box, 4), dtype=np.float32)  # [x0,y0,x1,y1]

            img_w, img_h = cfg_data.STD_SIZE[1], cfg_data.STD_SIZE[0]
    ...
    def get_wh_gt(self, bboxes):
        ...
        for k in range(bboxes.shape[0]):
            ...
            output_w = cfg_data.TRAIN_SIZE[1]
            ct = np.array(
                [(x0 + x1) / 2, (y0 + y1) / 2], dtype=np.float32)
            ct_int = ct.astype(np.int32)
            ind[k] = ct_int[1] * output_w + ct_int[0]
            if ind[k] > (cfg_data.TRAIN_SIZE[0] * cfg_data.TRAIN_SIZE[1]):
                print('ct1:', ct_int[1], "ct0:", ct_int[0], ind[k])
                print('*' * 100, 'error')
            assert (ct - ct_int)[0] >= 0 and (ct - ct_int)[1] >= 0
            assert ind[k] <= (cfg_data.TRAIN_SIZE[0] * cfg_data.TRAIN_SIZE[1])

            reg[k] = ct - ct_int
            reg_mask[k] = 1
        return wh, ind, reg_mask, reg
